...
const getMockBundesliga = () => {
  ...
  data.standings[0].table.forEach((row: any) => {
...
const getMockCLKnockout = () => {
  ...
  data.roundOf16.forEach((m: any) => {
    m.homeCrest = getCorrectCrest(m.home, m.homeCrest);
    m.awayCrest = getCorrectCrest(m.away, m.awayCrest);
  });
  data.quarterFinals.forEach((m: any) => {
    m.homeCrest = getCorrectCrest(m.home, m.homeCrest);
    m.awayCrest = getCorrectCrest(m.away, m.awayCrest);
  });
  data.semiFinals.forEach((m: any) => {
    m.homeCrest = getCorrectCrest(m.home, m.homeCrest);
    m.awayCrest = getCorrectCrest(m.away, m.awayCrest);
  });
  if (data.final) {
    data.final.homeCrest = getCorrectCrest(data.final.home, data.final.homeCrest);
    data.final.awayCrest = getCorrectCrest(data.final.away, data.final.awayCrest);
  }
  return data;
};
...
export function getCorrectCrest(name: string, originalCrest: string): string {
  if (!name) return originalCrest;
  if (name.includes('Freiburg')) {
    return `${base}/images/Titels/Feiburg .png`;
  } else if (name.includes('Frankfurt')) {
    return `${base}/images/Titels/Frankfurt .png`;
  } else if (name.includes('Bochum')) {
    return `${base}/images/Titels/Bochum .png`;
  } else if (name.includes('Heidenheim')) {
    return `${base}/images/Titels/Heidenheim.png`;
  } else if (name.includes('St. Pauli') || name.includes('St Pauli')) {
    return `${base}/images/Titels/St Pauli.png`;
  } else if (name.includes('Kiel')) {
    return `${base}/images/Titels/Kiel.png`;
  } else if (name.includes('Juventus')) {
    return `${base}/images/Titels/Juventus.png`;
  } else if (name.includes('Sporting') || name.includes('Lisbon') || name.includes('Lisabon')) {
    return `${base}/images/Titels/Sporting Lisabon.png`;
  } else if (name.includes('Atlético') || name.includes('Atletico')) {
    return `${base}/images/Titels/Atletico Madrid.png`;
  }
  return originalCrest;
}

function checkAndOverrideCrest(team: any) {
  if (!team) return;
  const name = team.name || team.shortName || '';
  team.crest = getCorrectCrest(name, team.crest || '');
}
